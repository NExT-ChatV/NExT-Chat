...
We also use OpenAI CLIP [ViT model](https://huggingface.co/openai/clip-vit-large-patch14-336/tree/main) as the visual encoder.
...
Please config the `vision_tower` in the `config/_base_/dataset/DEFAULT_TRAIN_DATASET.py` to the path of OpenAI CLIP, if you can not connect to huggingface.
```shell
...
## Training
Please config the `vision_tower` in the `config/_base_/dataset/DEFAULT_TRAIN_DATASET.py` to the path of [OpenAI CLIP](https://huggingface.co/openai/clip-vit-large-patch14-336/tree/main), if you can not connect to huggingface.
